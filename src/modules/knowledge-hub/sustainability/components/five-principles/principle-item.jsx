...
import ExploreLearning from './explore-learning-data.jsx';

export default function PrincipleItem({filteredPrincipleData}) {
  const [selectedItemId, setSelectedItemId] = useState(null);
  const [showExploreLearning, setShowExploreLearnig] = useState(false);
  const [exploreLearningData, setExploreLearningData] = useState({});
  const [activeTab, setActiveTab] = useState('explore');

  const handleInspired = (item, tab) => {
    setShowExploreLearnig(!showExploreLearning);
    setExploreLearningData(item);
    setActiveTab(tab);
  };
  
  return (
    <div className="principle">
      {!showExploreLearning && (filteredPrincipleData.length > 0 ? (
        filteredPrincipleData.map((item) => {
          return (
