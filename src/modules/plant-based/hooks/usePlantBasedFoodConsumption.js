...
import { plantBasedFoodConsumption, plantBasedFoodConsumptionKey } from '../api/request';
...
const usePlantBasedFoodConsumption = () => {
  ...
  const getKey = useCallback(async () => {
    const {data = [], status = ''} = await plantBasedFoodConsumptionKey();
    if(status === 'Success') {
      setFilter(data);
    }
  }, []);
  ...
  const getChartDataForCountry = useCallback(async () => {
    const {data={}, status=''}  = await plantBasedFoodConsumption({split:'country'});
    if(status === 'Success') {
      // format data
  ...
  const getChartDataForAge = useCallback(async () => {
    const {data={}, status=''}  = await plantBasedFoodConsumption({split:'age'});
    if(status === 'Success') {
      // format data
      const tempFormatData = formatPercentData(data);
...
export default usePlantBasedFoodConsumption;